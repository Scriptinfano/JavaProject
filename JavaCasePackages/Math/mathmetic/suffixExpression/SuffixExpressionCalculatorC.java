...
public class SuffixExpressionCalculatorC {
    ...
    /**
     * 后缀表达式计算器的构造器，传入一个代表中缀表达式的字符串，正则表达式会匹配其中的每一个元素
     *
     * @param theExpression 代表中缀表达式的容器
     */
    public SuffixExpressionCalculatorC(String theExpression) {
        expression = transform(theExpression);
    }

    /**
     * 后缀表达式计算器的构造器，传入String[]来构造，其中每一个元素都是中缀表达式中的一个元素
     *
     * @param theExpression 表达式
     */
    public SuffixExpressionCalculatorC(String[] theExpression) {
    ...
    /**
     * 将代表中缀表达式的字符串用正则表达式分割成String[]数组，数组中的每一个元素代表数字或四则运算符号或者左右括号
     *
     * @param expression 代表中缀表达式的字符串
     * @return 返回字符串数组
     */
    private String[] transform(String expression) {
        //TODO 找到一种方法忽略掉java的转义字符，让正则表达式正确匹配中缀表达式
        String originalRegex = "/[0-9]+|\\+|-|\\*|/|\\(|\\)/g";
        return expression.split("/[0-9]+|\\+|-|\\*|/|\\(|\\)/g");
    }

    /**
     * 执行整个运算过程，并返回运算结果
     *
     * @return double 返回的运算结果
     */
    public double run() {
