...
public class ExternalSorter {
    ...
    public void run() {
        //从序列中读取数据填满缓冲区
        //若没有任何车，则新建一辆车，从缓冲区中选择最小的元素并将该最小的元素添加到第一辆车（每辆车最大的元素就是最后一个上车的元素）
        //再从序列中填满缓冲区
        ...
        do {
            ...
            if (parkingLot.getLast().isEmpty()) {
                ...
                if (minValue.isPresent()) {
                    ...
                    cache.stream().min(Integer::compareTo).ifPresent(integer -> {parkingLot.getLast().addLast(integer);cache.remove(integer);});
                }
            }
        } while (!cache.isEmpty());
    }
